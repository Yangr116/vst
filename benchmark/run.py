...
# source file: https://github.com/open-compass/VLMEvalKit/blob/main/run.py
...
"""
    ...
    parser.add_argument('--model_path', type=str, default=None, help="The model path will load to the model")
...
def main():
    ...
    if args.config is not None:
        ...
        # yr resize start: change model path from arguments, config only direct the dataset
        if len(args.model) == 1 and args.model_path is not None:
            cfg['model'][args.model[0]]['model_path'] = args.model_path
            logger.warning(f"set {args.model[0]}'s path to {args.model_path}")
         # yr resize end
    else:
