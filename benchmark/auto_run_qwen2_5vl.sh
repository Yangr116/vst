...
torchrun --nnodes=$NNODES --nproc-per-node $NPROC_PER_NODE --node-rank $NODE_RANK \
  --master-addr=$MASTER_ADDR --master-port=$MASTER_PORT run.py --config config_qwen2_5.json --model_path "$model_path" --work-dir "$work_dir" 2>&1 | tee "work_dirs/$modified_string-log.txt"
